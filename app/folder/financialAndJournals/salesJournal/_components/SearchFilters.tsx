...
import { filterSalesJournal, setPageNumber, setQuery } from '@/app/Redux/Slices/folder/salesJournalSlice';

const SearchFilters = () => {

    const dispatch = useDispatch<AppDispatch>();
    const grandTotals = useSelector((state: RootState) => state.salesJournal.grandTotals);
    const pageLimit = useSelector((state: RootState) => state.salesJournal.pageLimitSalesJournal);
    const query = useSelector((state: RootState) => state.salesJournal.query);
  ...
  return `${year}-${formattedMonth}`;
        ...
        const handlePeriodChange = (field: string, value: any | boolean) => {
          ...
          if(Object.keys(queryObject).length !=0 && value){
            ...
            dispatch(filterSalesJournal({query:queryObject, limit:pageLimit, page: 1}))
          }else{
            delete queryObject.period;

            dispatch(setQuery(queryObject))
            dispatch(setPageNumber(1)); 
            dispatch(filterSalesJournal({query:queryObject}))
          }
      }

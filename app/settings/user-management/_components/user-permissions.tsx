...
// Define the access structure based on the provided data example
...
// Extended user type with accesses
...
interface UserPermissionsProps {
  ...
  onUpdateUserPermissions: (userId: string, accesses: UserAccesses) => void
}
...
export default function UserPermissions({ selectedUser, onUpdateUserPermissions }: UserPermissionsProps) {
  ...
  const saveChanges = () => {
    if (selectedUser && currentAccesses) {
      onUpdateUserPermissions(selectedUser.id, currentAccesses)
        ...
        <div className="space-y-4">
          {leftColumnPermissions.map((permission) => (
            <div key={permission.key} className="flex items-center justify-between">
              <span className={permission.key === "accessToStores" ? "text-purple-600 font-medium" : ""}>
                {permission.name}
              </span>
              <div className="flex items-center space-x-2">
                <Switch
                  checked={currentAccesses[permission.key]}
                  onCheckedChange={() => togglePermission(permission.key)}
                />
                <span className="text-sm text-gray-500 w-8">{currentAccesses[permission.key] ? "on" : "off"}</span>
              </div>
            </div>
          ))}
        </div>
